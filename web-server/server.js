...
app.get('/products/:id/did.json', (req, res) => {
  ...
  const data = readJSONFromFile(filePath);
  if (!data) return res.status(404).json({ error: `DID for product ${id} not found` });
  res.json(data);
});

app.get('/products/:id/credentials/materials.json', (req, res) => {
  const { id } = req.params;
  const filePath = path.join(DATA_DIR, 'products', id, 'credentials', 'materials.json');
  const data = readJSONFromFile(filePath);
  if (!data) return res.status(404).json({ error: `Verifiable Credential for product ${id} not found` });
  res.json(data);
});
