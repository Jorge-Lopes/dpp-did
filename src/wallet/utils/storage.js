...
export const storageManagement = async (storagePath, host) => {
  ...
  const getCredentialPaths = (productDid, credentialName) => {
    ...
    const credentialHost = 'https://' + productEndpoint + '/credentials/' + credentialName + '.json';
    const productReference = '/products/' + productEndpoint.slice(-1);
    const credentialStoragePath = productReference + `/credentials/${credentialName}.json`;
    const productStoragePath = productReference + '/did.json';
