...
export const createWallet = async (seed, options) => {
  const { method, storagePath, host } = options;

  const { storeDocument, getProductPaths, getCredentialPaths } = await storageManagement(storagePath, host);
  ...
  if (method === 'key') {
    ...
    await storeDocument(didDocument, '/did.json');
    return { keypair, did, didDocument, getDocument };
  }

  const { did, didDocument, methodFor } = await createDidWeb(publicKey, host);
  const { signSuite, contextSuite } = generateSignSuite(keypair, didDocument, methodFor);
  ...
  const createProduct = async () => {
    const { productHost, productStoragePath } = getProductPaths(productsRegistry.length);
  ...
  const createCredential = async (did, credential, credentialName) => {
    const { verifiableCredential, hash } = await _createCredential(credential, signSuite, documentLoader);

    const didDocument = await getDocument(did);
    const { credentialHost, credentialStoragePath, productStoragePath } = getCredentialPaths(did, credentialName);
    const updatedDidDocument = setService(didDocument, signSuite, { host: credentialHost, hash });
  ...
  return { did, didDocument, productsRegistry, getDocument, createProduct, createCredential, transferProductOwnership };
};
