...
export const createManufacturerWallet = async (seed, serverOptions) => {
  ...
  const { did, didDocument, methodFor } = await createDidWeb(keypair.publicKeyMultibase, host);
  ...
  const createCredential = async (did, credential, name, documentLoader) => {
    ...
    const endpoint = 'https://' + productEndpoint + '/credentials/' + name + '.json';

    const verifiableCredential = await product.addCredential(credential, signSuite, documentLoader, endpoint);

    const productReference = '/products/' + productEndpoint.slice(-1);
    const credentialStoragePath = storagePath + productReference + `/credentials/${name}.json`;
